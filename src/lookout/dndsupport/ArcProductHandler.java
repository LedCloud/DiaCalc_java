...
import tablemodels.ArcProductTableModel;
import java.awt.datatransfer.*;


//import lookout.MainFrame;

public class ArcProductHandler extends TransferHandler{
    ...
    public boolean importData(TransferHandler.TransferSupport info) {
        ...
        if (info.getComponent() instanceof JTable){
            ...
            prod.setCompl(false);
            ((ArcProductTableModel)tb.getModel()).addProduct(prod);
    ...
    if (c instanceof JTable){
        ...
        if (tb.getRowCount()>0 && !tb.getSelectionModel().isSelectionEmpty()){
            ...
            ProductInBase prod = new ProductInBase(((ArcProductTableModel)tb.getModel()).getProduct(
                    tb.convertRowIndexToModel(
                    row)));
            
            return new ArcProductTransferable( prod );
        }
    }
