...
public class ArcProductTableModel extends AbstractTableModel {
  ...
  private Vector<ProductInBase> products;
  private ArcProductManager mgr = new ArcProductManager();
  ...
  public ArcProductTableModel(int idGroup){
    groupid = idGroup;
    products = new Vector(mgr.getProductsFromGroup(idGroup));
 }
  ...
  public void addProduct(ProductInBase prod){
      ...
      products = new Vector(mgr.addProdInBase(prod));
      //fireTableRowsInserted(0,products.size()-1);
      fireTableDataChanged();
  }

  public void updateProduct(ProductInBase prod){
      products = new Vector(mgr.updateProductInBase(prod));
  ...
  public void deleteProduct(ProductInBase prod){
      if (products.size()>0){
        products = new Vector(mgr.deleteProductFromBase(prod));
